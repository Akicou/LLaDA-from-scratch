...
class ModelConfig():
    ...
    use_dsa: bool = False
    """
    If True, use DeepSeek Sparse Attention (DSA) for efficient long-context processing.
    """
    
    index_n_heads: int = 64
    """
    Number of heads for the lightning indexer in DSA.
    """
    
    index_head_dim: int = 128
    """
    Dimensionality of each indexer head.
    """
    
    index_topk: int = 2048
    """
    Number of top-k tokens to select per query in DSA.
    """
    
    index_use_fp8: bool = False
    """
    If True, use FP8 precision for indexer computations (requires H100+ GPU).
    Otherwise use BF16.
    """
    
    index_block_size: int = 128
    """
    Block size for quantization in FP8 indexer (if enabled).
    """
